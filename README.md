...
The current runtime artifact set passes both local verification and a fresh ETSI Conformance Checker sweep for all Baseline-B, Baseline-T, and Baseline-LT rows below. Baseline-LTA rollout has started, and the first local slices are CAdES-LTA plus ASiC-S-LTA built on embedded `archive-time-stamp-v2` support.
